...
bool MegalodonChecker::certificateSatSubsumptionResolutionStepSexpr(
  Kernel::Unit* unit,
  std::string& result,
  std::vector<MegalodonKernelSyntax::PrimitiveStep>* primitiveSteps)
{
  ...
  for (std::size_t mainParentIndex = 0; mainParentIndex < 2; ++mainParentIndex) {
    ...
    for (unsigned selectedIndex = 0; selectedIndex < mainParent->length(); ++selectedIndex) {
      ...
      for (unsigned sideLiteralIndex = 0; sideLiteralIndex < sideParent->length(); ++sideLiteralIndex) {
        ...
        if (sideSubstitutionSexpr != "(subst)") {
          sideSubstituteId = stepBase + "_side_subst";
          resolveSideParentId = sideSubstituteId;
          if (!certificateSubstituteStepSexpr(
                resolveSideParentId,
                sideParentId,
                sideParent,
        ...
        if (!sideSubstituteId.empty()) {
          localPrimitiveSteps.push_back(
            MegalodonKernelSyntax::primitiveClauseStep(
              "substitute",
              sideSubstituteId,
              {sideParentId},
              clauseFromLiterals(sideClauseLiterals),
              {{"substitution", sideSubstitutionSexpr}},
              sideSubstituteStep));
        }
        std::vector<std::string> actualLiterals;
        if (!appendCertificateClauseLiteralsSexpr(unit->asClause(), actualLiterals)) {
        ...
        auto emitSideSymmetry = [&](unsigned literalIndex, Kernel::Literal* substitutedLiteral) {
          ...
          const std::string sideSymmetryParentId = resolveSideParentId;
          const std::string sideSymmetryClause = clauseFromLiterals(sideClauseLiterals);
          ...
          const std::string sideSymmetryStep =
            "(equality_symmetry " + sexprQuote(sideSymmetryId)
            + " (parent " + sexprQuote(sideSymmetryParentId) + ")"
            + " (literal " + std::to_string(literalIndex) + ")"
            + " (result " + sideSymmetryClause + "))";
          primitivePrefix += sideSymmetryStep;
          primitivePrefix += "\n  ";
          localPrimitiveSteps.push_back(
            MegalodonKernelSyntax::primitiveClauseStep(
              "equality_symmetry",
              sideSymmetryId,
              {sideSymmetryParentId},
              sideSymmetryClause,
              {{"literal", std::to_string(literalIndex)}},
              sideSymmetryStep));
        ...
        const std::string resolveResultClause = clauseFromLiterals(resolveLiterals);
        const std::string resolveStep =
          "(resolve " + sexprQuote(resolveId)
          ...
          + " (result " + resolveResultClause + "))";
        primitivePrefix += resolveStep;
        primitivePrefix += "\n  ";
        localPrimitiveSteps.push_back(
          MegalodonKernelSyntax::primitiveClauseStep(
            "resolve",
            resolveId,
            {mainParentId, resolveSideParentId},
            resolveResultClause,
            {{"pivot_left", std::to_string(selectedIndex)}, {"pivot_right", std::to_string(sideLiteralIndex)}},
            resolveStep));
        ...
        while (!sameMultiset(currentLiterals, actualLiterals)) {
          ...
          for (unsigned left = 0; left < currentLiterals.size() && !factored; ++left) {
            for (unsigned right = left + 1; right < currentLiterals.size(); ++right) {
              ...
              const std::string factorResultClause = clauseFromLiterals(candidate);
              const std::string factorStep =
                "(factor " + sexprQuote(factorId)
                + " (parent " + sexprQuote(currentParentId) + ")"
                + " (literals " + std::to_string(left) + " " + std::to_string(right) + ")"
                + " (result " + factorResultClause + "))";
              primitivePrefix += factorStep;
              primitivePrefix += "\n  ";
              localPrimitiveSteps.push_back(
                MegalodonKernelSyntax::primitiveClauseStep(
                  "factor",
        ...
        result = primitivePrefix + finalStep;
        localPrimitiveSteps.push_back(
          MegalodonKernelSyntax::primitiveClauseStep(
            "substitute",
            stepBase,
...
void MegalodonChecker::printReplayExtra(Kernel::Unit* u, const InferenceRecorder::InferenceInformation* info)
{
  ...
  auto emitKernelV1 =
    [&](const std::string& kernelRule,
        ...
        const std::vector<MegalodonKernelSyntax::RenderedKernelTypedVariable>* skolemResultFormulaFreeVariables = nullptr) {
    ...
    if (skolemSourceFormulaFreeVariables != nullptr) {
      MegalodonKernelSyntax::setSkolemSourceFormulaFreeVariables(
        step,
        *skolemSourceFormulaFreeVariables);
    }
    if (skolemResultFormulaFreeVariables != nullptr) {
      MegalodonKernelSyntax::setSkolemResultFormulaFreeVariables(
        step,
        *skolemResultFormulaFreeVariables);
    }
  ...
  if (u->inference().rule() == Kernel::InferenceRule::SKOLEMIZE && !u->isClause()) {
    {
      ...
      auto quantifiedVariables =
        [&](Kernel::Formula* formula) {
          std::vector<MegalodonKernelSyntax::RenderedKernelQuantifiedVariable> result;
          std::function<void(Kernel::Formula*)> visit =
            [&](Kernel::Formula* current) {
              if (current == nullptr) {
                return;
              ...
              switch (current->connective()) {
                case Kernel::FORALL:
                case Kernel::EXISTS: {
                  const std::string kind = formulaConnectiveName(current->connective());
                  std::vector<std::pair<unsigned, Kernel::TermList>> vars;
                  Kernel::VSList::Iterator varIterator(current->vars());
      ...
      std::vector<MegalodonKernelSyntax::RenderedKernelTypedVariable>
        skolemResultFormulaFreeVariables;
      if (!resultFormula.empty()) {
        hasSkolemResultFormulaFreeVariables = true;
        skolemResultFormulaFreeVariables = freeVariables(u->getFormula());
      }
        ...
        if (!parent->isClause()) {
          ...
          if (certificateFormulaTermSexpr(parent->getFormula(), parentFormula)) {
            ...
            if (kernelParentIndex == 0) {
              ...
              hasSkolemSourceFormulaFreeVariables = true;
              skolemSourceFormulaFreeVariables =
                freeVariables(parent->getFormula());
            } else {
              MegalodonKernelSyntax::RenderedKernelSkolemMacroEdge edge;
      ...
      emitKernelV1(
        ...
        hasSkolemResultFormulaFreeVariables ? &skolemResultFormulaFreeVariables : nullptr);
    }
    std::vector<std::string> fields;
    fields.push_back("rule=" + Kernel::ruleName(u->inference().rule()));
    std::string targetText;
        ...
        if (u->inference().rule() == Kernel::InferenceRule::FORWARD_SUBSUMPTION_RESOLUTION
          ...
          if (certificateSubstitutedResolutionStepsSexpr(
            ...
            && MegalodonKernelSyntax::appendPrimitiveExpansionChainFields(
              ...
              primitiveSteps,
              "u" + std::to_string(u->number()))) {
          } else if (certificateSatSubsumptionResolutionStepSexpr(
                u,
                primitiveExpansion,
                &primitiveSteps)
            && MegalodonKernelSyntax::appendPrimitiveExpansionChainFields(
